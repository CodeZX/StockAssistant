...
@interface SA_GuideViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation SA_GuideViewController
...
- (void)requestInfos
{
    ...
    AFHTTPSessionManager *manager  = [AFHTTPSessionManager manager];
    [manager GET:@"http://149.28.12.15:8080/gp/manual/list" parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        
        [MBProgressHUD hideHUD];
        if ([responseObject[@"code"] intValue] == 200) {
            
            NSMutableArray *array = [SA_NewsModel mj_objectArrayWithKeyValuesArray:responseObject[@"retData"]];
        ...
        [MBProgressHUD hideHUD];
        [MBProgressHUD showSuccess:@"网太弱了，请稍后重试！"];
    }];
}
...
//    cell.videoModel = self.todayPlayArray[indexPath.row];
    cell.newsModel = self.todayPlayArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        SA_NewsModel *model = self.todayPlayArray[indexPath.row];
    infoModel.iId = [model.m_id integerValue];
    infoModel.title = model.name;
    infoModel.content = model.content;
    infoModel.date = @"2018-12-13";
    infoModel.synopsisPic = model.img;
    infoModel.urlString = @"111";
    infoModel.readNumber = 10;
//    infoModel.pictures = @[@"111",@"333"];
