...
- (void)viewDidLoad {
  [super viewDidLoad];
    self.title = [self.conversation valueForKey:@"name"];
  ...
  _imuiInputView.inputViewDelegate = self;
    ...
    [JMSGConversation allChatRoomConversation:^(id resultObject, NSError *error) {
        messageArray = resultObject;
//        NSString *info = [NSString stringWithFormat:@"chat room conversation count:%lu \n, %@", (unsigned long)array.count, array];
...
- (void)sendTextMessage:(NSString * _Nonnull)messageText {
  JMSGTextContent *content = [[JMSGTextContent alloc] initWithText: messageText];
    ...
    JMSGMessage *message = [JMSGMessage createChatRoomMessageWithContent:content chatRoomId:[self.conversation valueForKey:@"roomID"]];
...
//  }
    ...
    [MBProgressHUD showSuccess:@"网络错误！请重试！"];
}
